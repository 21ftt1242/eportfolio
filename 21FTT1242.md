...
1. **[Setup](#setup)**
    ...
    i) Creating SSH Key using terminal
    
    ii) Creating a Digital Ocean Droplet

2. **Root login using terminal**
    
    i) Make new non-root user
    
    ii) Grant Administrative Privileges (Sudo)
    
    iii) Enabling external access for non-root user
    
    iv) Setting up firewall
    
    v) SSH Server Configuration
    
    vi) Open Custom Port

3. **Creating config file for shortcut**

4. **Login as non-root user**
    
    i) Install and enable Nginx
    
    ii) Setting up server blocks
    
    iii) Changing Timezone



  * [1. Setup](#1-setup)
    + [Creating SSH Key using terminal](#creating-ssh-key-using-terminal)
    + [Creating a Digital Ocean Droplet](#creating-a-digital-ocean-droplet)
  * [2. Root Login using terminal](#2-root-login-using-terminal)
    + [Make new non-root user](#make-new-non-root-user)
    + [Grant Administrative Privileges (Sudo) for non-root user](#grant-administrative-privileges--sudo--for-non-root-user)
    + [Enabling external access for non-root user](#enabling-external-access-for-non-root-user)
    + [Setting up firewall](#setting-up-firewall)
    + [SSH Server Configuration](#ssh-server-configuration)
    + [Open Custom Port](#open-custom-port)
  * [3. Create config file for shortcut](#3-create-config-file-for-shortcut)
  * [4. Login as non-root user](#4-login-as-non-root-user)
    + [Install and enable Nginx Full](#install-and-enable-nginx-full)
...
## 1. Setup
### Creating SSH Key using terminal 
...
# password 21ftt1242
```
### Grant Administrative Privileges (Sudo) for non-root user
```
# usermod -aG sudo ftt1242
```
### Enabling external access for non-root user
```
