...
4. Login as non-root user
    ii) Install and enable Nginx
    iii) Setting up server blocks
    iv) Changing Timezone
...
1vCPU, 25GB SSD 1000GB Transfer)`
    ...
    viii) Add Sir Bahit's SSH Key: `Copy and Paste Sir Bahit's SSH Key and name it ` ***`sop_as1_sirbahit`***
    ix) Hostname: `sop-as1`
4. Create Droplet
...
1. Update local package index and install Nginx
```
$ sudo apt update
